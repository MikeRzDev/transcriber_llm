...
The live latency target concerns **speech onset to the first text update**, including after a long quiet wait. Native live feeds are capped at 100 ms; a short first resampled packet is sent immediately. Voxtral is warmed before microphone capture and uses its supported 80 ms transcription-delay setting. The terminal applies incoming text before drawing and polls live events every 20 ms. This low-delay profile prioritizes earlier partials and can be less accurate than the model's recommended 480 ms delay; see [Mistral's delay settings](https://huggingface.co/mistralai/Voxtral-Mini-4B-Realtime-2602#recommended-settings). Qwen and Parakeet continue to use speech windows of up to four seconds.

The microphone monitor distinguishes **First text** (approximate detected speech onset through the completed terminal draw of meaningful text) from **s/audio s** (a rolling processing-speed average). A processing-speed value below 1 alone does not prove sub-second text latency. [The eight-minute idle software-path check](benchmarks/voxtral-idle-latency.md) measured 0.836 seconds through the completed terminal test-backend draw against a 0.9-second target; physical microphone and display timing were not included. Idle requests are excluded from that speed average. Real latency also depends on the input device, speech, noise, and competing workloads. The 0.9-second limit is an explicit target, not a hard real-time guarantee: a measured miss is shown as `> 0.9s` and logged. Steady-state transcript redraws cache committed history and copy only visible rows; partial updates do not re-wrap past speech. Resizing or changing speaker labels reflows history once. Process-resource sampling runs asynchronously and cannot block the UI on a process scan.

Native live mode suspends inference during quiet input, keeps model weights loaded, and retains 250 ms of pre-roll for speech onset. Input energy is checked in 20 ms frames using the microphone monitor's quiet threshold. A one-second quiet tail flushes pending words and releases streaming context; the next utterance starts with fresh context and absolute microphone timestamps. Very low input levels can remain below this energy threshold. Active context is also flushed at the 20-second limit without reloading weights. Quiet input outside the pre-roll buffer is not decoded, and a forced context boundary can affect recognition near that boundary.

Legacy Voxtral 4-bit checkpoints are optimized once in memory: remaining large embedding/adapter matrices use the existing 4-bit format, and floating parameters/scales use BF16. Model files remain unchanged; fp16 and other model variants keep their selected precision. [The earlier throughput study](benchmarks/voxtral-optimization.md) measured the improvement with one-second feeds; it did not establish first-text latency after idle. The in-memory capture queue is bounded at about 120 seconds. If it fills, recording stops gracefully, finishes queued speech, and exports. A larger queue cannot compensate for sustained inference slower than realtime. Pin Qwen's language with `-l es`; native Voxtral detects its own language.

The **m** model selector shows measured live suitability for this Mac: **good fit** (at least 20% compute headroom), **borderline** (keeps up with little headroom), or **too slow for this Mac** (a tested repetition fell behind). Each rating shows processing seconds per audio second and its tested feed size. Incompatible models say **live unavailable**; models without matching measurements say **not benchmarked**. These labels concern live transcription; models remain selectable for file transcription. Ratings are stored beside the app config in `live-benchmarks.json`, checked against the Mac's hardware and model file sizes/modification times, and refreshed whenever you press **m**. The displayed feed size identifies the saved throughput test. Live native feeds use the 100 ms latency limit even when a larger batch gave better benchmark throughput.
