...
pub(super) fn draw_recording(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let speed = if live.inference_idle && live.recording {
        "Waiting for speech · model ready".into()
    } else {
        match live.inference_rtf {
            Some(rtf) => format!(
                "{rtf:.2}s/audio s · {}",
                if rtf > 1.0 {
                    "falling behind"
    ...
    frame.render_widget(
        Paragraph::new(format!(
            ...
            speed,
            live.first_text_ms.map(|ms| format!("{:.2}s{}", ms / 1000.0, if ms > 900.0 { " > 0.9s" } else { "" })).unwrap_or_else(|| "—".into()),
        ))
